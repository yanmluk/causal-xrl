...
from stratvis.lib.r_to_py.r_to_py import init_R
from stratvis.lib.utils.utils import convert_to_numpy, convert_and_expand
...
class BARTRegressor:
    ...
    def __init__(self):
        ...
        bart_train <- function(X, y) {        
          ...
          bart_machine <- bart(X, y, nskip=200, ndpost=125, nchain=4, nthread=4, 
                               seed=98, keeptrees=T, verbose=F)
          # save trained bart_machine along with trees (trees are not saved by default)
          invisible(bart_machine$fit$state)
    ...
    def is_causal_ITE(self, X, trt_col, trt=1., ctrl=0., additive=False):
        """ Determines if there is a (possibly heterogeneous) causal effect. 
        ...
        lci, uci = np.quantile(ite_per_model, [0.025, 0.975])
    ...
    def CITest(self, X, y, trt_col):
        ...
        """
        X_train, X_est, y_train, y_est = train_test_split(X, y, test_size=0.5, random_state=99)
    ...
    def CITestNew(self, X_train, y_train, X_test, y_test, trt_col):
        y_hat = self._r_sample(X_train, y_train, X_test)

        # drop the trt_col
        X_train_cf = np.delete(X_train, trt_col, axis=1)
        X_est_cf = np.delete(X_test, trt_col, axis=1)
